...
// major = { number }
// minor = { number }
// extra = { 'a':alpha, 'b':beta, 'rc': release candidate, 'r': release,
// 's':stable }
...
#if DEBUG
  Assertions = Enabled // enabled or disabled assertions (these asserts work
                       // both in DEBUG and RELEASE builds)
#else
  Assertions = Disabled // enabled or disabled assertions (these asserts work
                        // both in DEBUG and RELEASE builds)
#endif
};
...
enum Format {
  ...
  TaggedXML = 4 // XML output, tagged XML format. see
                // https://github.com/hjiang/jsonxx/issues/12
};
...
public:
  ...
  const T &get(const std::string &key,
               const typename identity<T>::type &default_value) const;
  ...
  std::string xml(unsigned format = JSONx,
  ...
  template <size_t N> Object(const char (&key)[N], const Value &value) {
    import(key, value);
  }
...
public:
  ...
  template <typename T>
  [[deprecated("this type is not natively supported by jsonxx, therefore its "
               "value will be converted to 'null'")]] void
  import(const T &t) {
    reset();
    type_ = INVALID_;
#ifdef DEBUG
    std::cerr << "[WARN] No JSONXX support for " << typeid(t).name()
              << std::endl;
#endif
  }
...
#define $number(TYPE)                                                          \
  ...
  $number(char) $number(int) $number(long) $number(long long)
      $number(unsigned char) $number(unsigned int) $number(unsigned long)
          $number(unsigned long long) $number(float) $number(double)
              $number(long double)
#undef $number
  ...
  template <typename T> void import(const std::vector<T> &array) {
    Array a;
  ...
  template <typename T> Value(const T &t) : type_(INVALID_) { import(t); }
  template <size_t N> Value(const char (&t)[N]) : type_(INVALID_) {
    import(std::string(t));
  }
...
public:
  enum { NUMBER_, STRING_, BOOL_, NULL_, ARRAY_, OBJECT_, INVALID_ } type_;
  union {
...
template <typename T> T &Array::get(unsigned int i) {
  JSONXX_ASSERT(i < size());
  Value *v = values_.at(i);
  return v->get<T>();
}

template <typename T> const T &Array::get(unsigned int i) const {
  JSONXX_ASSERT(i < size());
...
template <typename T>
const T &Array::get(unsigned int i,
                    const typename identity<T>::type &default_value) const {
  if (i < size()) {
    const Value *v = values_.at(i);
    if (v->is<T>()) {
...
template <typename T> bool Object::has(const std::string &key) const {
...
template <typename T> const T &Object::get(const std::string &key) const {
  JSONXX_ASSERT(has<T>(key));
  return value_map_.find(key)->second->get<T>();
}

template <typename T>
const T &Object::get(const std::string &key,
                     const typename identity<T>::type &default_value) const {
...
template <> inline const Value &Value::get<Value>() const { return *this; }

template <> inline bool &Value::get<Boolean>() {
  JSONXX_ASSERT(is<Boolean>());
  return bool_value_;
}

template <> inline std::string &Value::get<String>() {
